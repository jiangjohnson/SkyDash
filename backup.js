...
var navmodule = {
    ...
    if(data.length > 0){      
      ...
      for(var i in PerformanceChannels){
        ...
        if(PerformanceChannels[i].ChannelName != ''){
          ...
          output += '<td class="text-center"><img width="25" src="channel/globe.png" class="img-responsive">&nbsp;&nbsp;TOTAL</td>';
        }        
          ...
          for(var p in PerformanceChannels[i].Channels){
            if(PerformanceChannels[i].Channels[p].PlatFormName == Formname[x]){
              ...
              if(PerformanceChannels[i].ChannelID != -1){
                ...
                output += '<td class="'+ activecell +'" id="'+PerformanceChannels[i].ChannelID+'" value="'+ PFormID +'" data-id="'+Formname[x]+'" data-value="TOTAL">'+ avgval +'</td>'
              }                            
  ...
  */
  ...
  init_trending : function(ProgTitleID,ChGroupID,ChID,PtypeID,Prange,PFormID,TrendingTitle){
    ...
    if($('#barnum').val() != '' && $.isNumeric($('#barnum').val()) == true && $('#barnum').val() <= 15){
    ...
    $.get(window.location.href + "trending", request, function(data){
      ...
      var uniqLabel = _.uniq(label)
      var axislabel = []
      for(var i in _.uniq(label)){
        if(Number(i) < barnum){
