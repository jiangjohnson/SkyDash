      ...
      var promise = $.get(window.location.href + "channelgroup", request).then(function(data){
        ...
        */
        return $.get(window.location.href + "periodrange", request);
          ...
          var ChannelGroupID = $('#channelview').val() == 1 ? $('#channelgroup').val() : -1;
  ...
  */
  init_ChannelPerformance : function(data,ChannelGroupID){
    if(data.length > 0){
      ...
      var PlatformName = [];
      for(var i in data){
        var TempChannelID = ChannelGroupID != -1 ? data[i].ChannelID : data[i].ChannelGroupID;
      ...
      var output = '<table class="table-bordered table-channelperformance" id="table-channelperformance" width="100%">';
        output += '<thead class="bg-white"><tr><td width="220"></td>';
      for(var i in Formname){
        output += '<td class="text-center"><img height="50" src="channel/'+ Formname[i] +'.png" alt="'+ Formname[i] +'"></td>';
      }
        output += '<td class="text-center"><img width="40" src="channel/globe.png" class="img-responsive"></td>';
      ...
      for(var i in PerformanceChannels){
        ...
        output += '<td class="'+ activeTotalCell +'" id="'+PerformanceChannels[i].ChannelID+'" value="-1">'+ Number(totalAvgVal).toFixed(2) +'</td>';
        output += '</tr>';
      }
      
      output += '<tr>'
      output += '<td><img width="'+ size +'" src="channel/globe.png" class="img-responsive"><strong class="text-right">TOTAL</strong></td>';
      output += '<td class="text-center" id="'+PerformanceChannels[i].ChannelID+'" value="-1" class="text-left">'+ totalLinear.toFixed(2) +'</td>'
  ...
  init_ProgrammePerformance : function(ChGroupID,ChID,PFormID,PtypeID,Prange,ChannelName,FormName,SumSort){
    ...
    $.get(window.location.href + "programmeperformance", request, function(data){
      ...
      output += '<thead class="bg-white">'+
        '<tr>'+
          '<td><img height="30" src="logo/'+ cname.replace('+', '-') +'.png">'+ form +'</td>'+
          '<td></td>'+
          '<td></td>'+
          '<td></td>'+
        '</tr>'+
      ...
      $('#table-programmeperformance tr').each(function(){
        var ChannelGroupID = $('#channel').val() == 1 ? $('#channelgroup').val() : -1;        
        $(this).click(function(){
          if($(this).attr("id")!= undefined){
            $('#table-programmeperformance tr').each(function(){
              $(this).removeClass('active')
  ...
  init_trending : function(ProgTitleID,ChGroupID,ChID,PtypeID,Prange,PFormID,TrendingTitle){
    ...
    $.get(window.location.href + "trending", request, function(data){
      ...
      if(PFormIDs.length == 1){
        ...
        var topxAxis = []
        console.log("length"+dataseries[0].data.length)
        for(var i in dataseries){
          for(var x in dataseries[i].data){
            if(topxAxis.length < dataseries[0].data.length){
              topxAxis.push(dataseries[i].data[x])
            }else{
              topxAxis[x] = Number(topxAxis[x]) + Number(dataseries[i].data[x])
            }
          }
        }
...
/*
===== show/hide when channelview type ID is 1 or not
*/
...
$(function(){
  ...
  navmodule.initnav($('#channelview').val(),'channelgroup',$('#periodtype').val())
  /*
